...
class Machine{
    ...
    run(input){
        ...
        if(input == "" || input == null){
            ...
            return `İfade giriniz-Enter values from alphabet = ${this.input_alphabet} Çıktı-your output will be here`;
        }
        ...
        input.forEach(element => {
            ...
            if(edgetonext == null){
                console.log("ALFABE İÇİNDE OLMAYAN DEĞER GİRMEYİNİZ/ERROR INPUT VALUES NOT IN ALPHABET",iter_state,element,edgetonext);
...
function gen_table_option_output(table_name,number_of_states,alphabet,alp_output){
    ...
    ${gen_table_head_elements(["STATES","OUTPUT/ÇIKTI"])}
    <tbody>`
    for (let index = 0; index < number_of_states; index++) {
        let optable = new Array()
            let id = `O${index}`
...
function test (){
    newtable = gen_table_option("TRANSITION/GEÇİŞ TABLE",number_of_states,input_alphabet)
    document.getElementById("table-main").innerHTML = newtable
    newtable = gen_table_option_output("OUTPUT/ÇIKIŞ TABLE",number_of_states,input_alphabet,output_alphabet)
    document.getElementById("table-main").innerHTML += newtable
...
function generate_tt_run(){
    headinput = `<th scope="col">OLD/Eski STATE</th>`
    new_state_output = '<th scope="col">OUTPUT/Çıktı</th><th scope="col">NEW/Yeni STATE</th>'.repeat(input_alphabet.length)
    input_alphabet.forEach(element => {
    ...
    <thead class="thead-dark">
      <tr>
        <th scope="col" colspan="100">Transition/Geçiş Table and Output/Çıktı Table</th>
      </tr>
    </thead>
    <thead>
      <tr>
  ...
  </table>  <small class="text-primary"> <br>
  </small>
